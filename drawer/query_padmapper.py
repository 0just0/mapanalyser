...
def download(fname):
    ...
    print("Visit:")
    print(
        'https://www.padmapper.com/apartments/belmont-ma/belmont-hill?box=-71.1993028524,42.396054506,-71.1761285665,42.4262507215&property-categories=apartment'
    )
    print(
        "Inspect the networking, find a pins request, copy request as curl and paste here."
    )
    inp = input("> ")
    if "--data-binary" not in inp:
        raise Exception(
            "Something looks wrong.  Was that the curl version of a pins request?"
        )

    print("Run:")
    print(
        inp.split("--data-binary")[0] +
        '--data-binary \'{"bedrooms":[0,1,2,3,4,5],"limit":10000,"maxLat":%s,"minLat":%s,"maxLng":%s,"minLng":%s,"offset":0,"propertyCategories":["apartment"]}\' --compressed -o %s'
