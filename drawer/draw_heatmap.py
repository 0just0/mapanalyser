...
# set boundaries = test for Boston
MIN_LAT = 42.255594
MAX_LAT = 42.4351936
MIN_LON = -71.1828231
MAX_LON = -70.975800
...
def pixel_to_ll(x, y):
    """
    Convert pixels to Longtitude-Latidute data
    """
...
def load_prices(fs):
    ...
    slope, y_intercept = linear_regression(
        [(bedrooms, rent) for (bedrooms, rent, lat, lon) in raw_prices])
    print("slope =", slope)
    ...
    prices = [(rent / (bedrooms + num_phantom_bedrooms), lat, lon, bedrooms)
              for (bedrooms, rent, lat, lon) in raw_prices]
...
def color(val, buckets):
    """
    Colorize pixel depends on price
    """
    if val is None:
...
gaussian_negative_inverse_twice_variance_squared = -1 / (
    2 * gaussian_variance * gaussian_variance)
...
def gaussian(prices, lat, lon, ignore=None):
    ...
    for price, plat, plon, _ in prices:
        ...
        weight = gaussian_a * math.exp(
            distance_squared(lat, lon, plat, plon) *
...
def start(fname):
    ...
    bedroom_categories = list(
        sorted(set(bedrooms for _, _, _, bedrooms in priced_points)))
    adjustments = {}
    for bedroom_category in bedroom_categories:
        ...
        for i, (price, plat, plon, bedroom) in enumerate(priced_points):
            ...
            predicted_price = gaussian(
                priced_points, plat, plon, ignore=(plat, plon))
    ...
    all_priced_areas = sorted(all_priced_areas)
